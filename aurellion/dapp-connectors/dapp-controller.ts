...
export const { address, chainId, isConnected } = useWeb3ModalAccount()
export const { walletProvider } = useWeb3ModalProvider()
var ethersProvider: BrowserProvider;
if (walletProvider) {
    ethersProvider = new BrowserProvider(walletProvider)
    console.log(ethersProvider)
} else console.error(`Error when trying to initialise ethersProvider with walletProcider ${walletProvider}`)
...
export const jobCreation = async (locationData: ParcelData, recipientWalletAddress: string) => {
     try {
       ...
       const contract = new ethers.Contract(process.env.EXPO_PUBLIC_AUSYS_CONTRACT_ADDRESS, contractABI, signer);
...
export const fetchCustomerJobs = async () => {
     ...
     try {
       ...
       try {
         contract = new ethers.Contract(process.env.EXPO_PUBLIC_AUSYS_CONTRACT_ADDRESS, contractABI, signer);
       } catch (error) {
         console.error(
           `failed to instantiate contract object at with Contract Address: ${process.env.EXPO_PUBLIC_AUSYS_CONTRACT_ADDRESS} contractABI: ${contractABI} signer:${signer}`
         );
         throw error;
       }
