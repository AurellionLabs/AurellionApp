...
import { ParcelData } from '../common/types/types';


export const useJobCreation = (locationData: ParcelData | undefined, senderWalletAddress: string | undefined, recipientWalletAddress: string | undefined) => {
  const { config } = usePrepareContractWrite({
    address: REACT_APP_AUSYS_CONTRACT_ADDRESS as `0x${string}`,
    abi: contractABI,
    functionName: 'jobCreation',
    args: [senderWalletAddress, recipientWalletAddress, locationData, 1, 10],
  });

  const { write, error, isLoading, isError } = useContractWrite(config);

  if (error) {
    console.error('Error in useContractWrite:', error);
  }

  return { write, isLoading, isError, error };
};
