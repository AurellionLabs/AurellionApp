...
import { fetchDriverUnassignedJourneys } from "@/dapp-connectors/dapp-controller";
export default function AcceptJourney() {
  const { isDarkMode } = useMainContext();
  const { setAvailableJourneys, availableJourneys } = useDriverContext();

  useEffect(() => {
    const getAvailableJourneys = async () => {
      const journeys = await fetchDriverUnassignedJourneys();
